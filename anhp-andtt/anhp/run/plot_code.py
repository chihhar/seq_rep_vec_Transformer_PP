...
def plot_eventId_pred(args):
    with open(args.PathResult+"GT_his", 'rb') as f:
        GT_his = pickle.load(f)
    with open(args.PathResult+"pred_his", 'rb') as f:
        pred_his = pickle.load(f)
    print(f"len:{len(GT_his)}")
    plt.clf()
    plt.figure(figsize=(10,4)) 
    plt.xlabel("event ID",fontsize=18)
    plt.ylabel("elapsed time",fontsize=18)
    plt.plot(range(100),GT_his[2:102],label="ground-truth")
    plt.plot(range(100),pred_his[2:102],c="r",label="anhp",linestyle="dashed")
    
    plt.legend(fontsize=18, loc='upper right')
    plt.savefig(args.PathResult+"anhp_eventId_hispred_.svg", bbox_inches='tight', pad_inches=0)
    plt.savefig(args.PathResult+"anhp_eventId_hispred_.pdf", bbox_inches='tight', pad_inches=0)
    plt.savefig(args.PathResult+"anhp_eventId_hispred_.png", bbox_inches='tight', pad_inches=0)
    GT=np.array(GT_his)
    PD=np.array(pred_his)
    print(f"RMSE:{np.sqrt(((GT-PD)**2).mean())}, std:{np.sqrt(np.std((GT-PD)**2))}")
def main():
