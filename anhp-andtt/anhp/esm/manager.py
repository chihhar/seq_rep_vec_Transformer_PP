...
class Manager:
    ...
    def get_dataloader(self, args):
        ...
        return loaders
    def event_ll_plot(self,model:XFMRNHPFast, dataLoader, mode):
        model.eval()
        print(mode)
        test_data=dataLoader.__iter__()
        with open(f"/data1/nishizawa/Desktop/Transtrans/Transformer-Hawkes-Process/pickled/proposed/h_fix05/target_intensity", "rb") as file:
            target_history=pickle.load(file)
        for i in range(30):
            print(i)
            if i ==0:
                test_datax = test_data.next()
                test_time=test_datax[0]
                event_type=test_datax[1]
                vis_mask=test_datax[2]
                att_mask=test_datax[3]
                mask=test_datax[4]
            else:
                test_time=torch.cat((test_time,test_data.next()[0]))
        input=test_time[:,:-1]
        target=test_time[:,-1:]
        target_history=np.array([x.cpu().numpy() for x in target_history])
        target_tau=np.ediff1d(target_history)
        cumsum_tau = torch.cumsum(target,dim=0)
        loop_start=0
        loop_num=5000
        t_min=0
        t_max=(target.sum()+math.pow(10,-9)).item()
        ...
        with torch.no_grad():
            for t in range(loop_start,loop_num):
                ...
                #eve_ll
                intensities_for_sampletarget=model.compute_intensities_at_sampled_times(event_type[:,:29].cuda(),now_input.cuda(),variation_target.cuda())
                inten=intensities_for_sampletarget.sum(dim=-1)
                log_likelihood_history=np.append(log_likelihood_history,inten.cpu().numpy())
        pdb.set_trace()
        with open(f"/data1/nishizawa/Desktop/Transtrans/Transformer-Hawkes-Process/pickled/anhp/calc_intensity", 'wb') as file:
    ...
    def run_prediction(self, model:XFMRNHPFast, dataLoader):
        ...
        for _batch in tqdm(dataLoader, desc=f"   (Pred)    ", leave=False,dynamic_ncols=True, mininterval=2):
            ...
            for i in range(num_batch):
                ...
                time_uncond = float(torch.sum(accepted_times * weights))#重み*アクセプト時刻100コの総和=未知の時間
                ...
                )[0, 0]#その時刻での強度=model(イベント種類履歴、経過時刻履歴29個、_time_prefix=経過時間1個(未来)167行と対称)
                ...
                se_his.append(((time_uncond-next_event_time)**2))
                pred_his.append(time_uncond)
                GT_his.append(next_event_time)
                if verbose:
                    print(
