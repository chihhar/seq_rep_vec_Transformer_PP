...
from utils.log import LogWriter, LogReader
...
class Tester(Manager):
    ...
    def run(self):
        ...
        tic = time.time()
        self.event_ll_plot(self.model, dataloader, "eval")
        #pdb.set_trace()
        # testing with loglik
        ...
        if self.args.Predict:
            ...
            all_res_pred, rmse, GT_his, pred_his,std = self.run_prediction(self.model, dataloader)
            # finish
            all_res['pred'] = all_res_pred
        #len(all_res["loglik_token"])242
        #pdb.set_trace()#all_res["loglik"] len(GT_his)243
        with open(self.args.PathResult, 'wb') as f:
            pickle.dump(all_res, f)
        with open(self.args.PathResult+"rmse", 'wb') as f:
            pickle.dump(rmse, f)
        with open(self.args.PathResult+"GT_his", 'wb') as f:
            pickle.dump(GT_his, f)
        with open(self.args.PathResult+"pred_his", 'wb') as f:
            pickle.dump(pred_his, f)
        with open(self.args.PathResult+"std", 'wb') as f:
            pickle.dump(std, f)
        print("\ntesting finished")
